...
% 1) Estimate a surge model (incidence model) from the average surge behavior across the
...
% 5) Provide a Forecast for season 2026, based on the model of the average surge and
...
% Known data available for forecast for 2026 season  
ind_EW25_2025=649+52+52+52; % sample index of EW 25 of 2025

dcases=M{:,2};   % time-series of the surges across the EWs 

dcases_orig=dcases;  % stores the original time-series up to EW 25 of 2025

dcases=dcases(1:ind_EW25_2025);  % crops time-series at EW 25 of 2025 (including), if needed
...
% Initial parameters of the incidence model
L=120000;  
k=0.3;
n0=26;  % time-shift
P=[L,k,n0];
...
% up to EW 39 of 2025 (including)


% Note: dcases_rec goes to EW 39 of 2025, i.e., it includes a
% 'deterministic' forecast of EW 26 to EW 39 of 2025
forecast_determ=dcases_rec(ind_EW25_2025+1:end);


% A simple predictor of the dengue cases in Forecast 26 is to generate
...
MC=10000;  % number of Monte Carlo runs for forecast surges in 2026
% Note that forecast surge from EW 26 to 39 of 2025 may be best taken from the fitted
% surge around EW 15 of 2025.
...
% surge model from EW 40 of 2025 to EW 52 of 2026

forecast_cases=zeros(length(surge_model),MC);  % matrix to store realizations of the forecast surges 
% from EW 40 of 2025 to EW 52 of 2026

for kk=1:MC
    forecast_cases(:,kk)=g_MC(kk)*surge_model;
end
...
PP=prctile(forecast_cases',set_prctile); % calculates the percentiles and stores in PP
...
median_cases=mean_cases_forecast(1+(1:52)); % median forecast from EW 41 2025 to EW 40 2026
...
writetable(T,['..\planilhas\forecast26_Surge_Model_',UF,'.csv'],'Delimiter',',');
...
print(['..\plots\forecast26_SModel_',UF],'-dpdf')
