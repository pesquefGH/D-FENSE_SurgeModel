...
% 5) Provide a 'Validation 2' Forecast based on the model of the average surge and
% a simple statistical analysis of the set of gains.  



M = readtable(['DengueSprint2025_AggregatedData_',UF,'.csv']); % reads aggregated
% data from the chosen state, from 2010 to 2024

PP=52;  % period of 52 Epidemic Weeks (EW)

% Validation 2 
ind_EW25_2023=649+52; % sample index of EW 25 of 2023 

dcases=M{:,2};   % time-series of the surges across the EWs 

dcases_orig=dcases;  % stores the original time-series up to EW 52 of 2024
...
% up to EW 39 of 2023 (including)


% Note: dcases_rec goes to EW 39 of 2023, i.e., it includes a
% 'deterministic' forecast of EW 26 to EW 39 of 2023
forecast_determ=dcases_rec(ind_EW25_2023+1:end);


% A simple predictor to forecast the cases in Validation 2 is to generate
...
MC=10000;  % number of Monte Carlo runs for forecast surges in 2024
% Note that forecast surge from EW 26 to 39 of 2023 may be best taken from the fitted
% surge around EW 15 of 2023.
...
cases=dcases_orig(indf_ini:indf_end);

median_cases=mean_cases_forecast(1+(1:52)); % median forecast from EW 41 2023 to EW 40 2024
...
writetable(T,['..\planilhas\validation_2_Surge_Model_',UF,'.csv'],'Delimiter',',')
...
figure
subplot(221)
plot(EW_index,cases,'linewidth',2); % plot known sequence of cases from EW 41 2023 up to EW 40 of 2024
hold on;
...
print(['..\plots\Validation2_SModel_',UF],'-dpdf')
